...
Execute the /gh-release-push command. The full workflow follows; treat any
...
1. Run the repo's consistency check — `mage audit`, or `mage analyze` where the target is named that way. If it fails, fix the problems and re-run (up to 3 attempts). Stop if still failing after 3 attempts.
2. Run `mage test`. If it fails, fix the problems and re-run (up to 3 attempts). Stop if still failing after 3 attempts.
